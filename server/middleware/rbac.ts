 ...
 */
export async function getUserAssignedCustomerIds(userId: string): Promise<string[]> {
  try {
 ...
 */
export async function getUserAccessibleSupplierIds(userId: string): Promise<string[]> {
  try {
 ...
 */
export async function getUserAccessibleBookingIds(userId: string): Promise<string[]> {
  try {
    const customerIds = await getUserAssignedCustomerIds(userId);
 ...
 */
export async function getUserAccessibleInvoiceIds(userId: string): Promise<string[]> {
  try {
 ...
 */
export function isAdminUser(user: AuthRequest['user']): boolean {
 ...
 */
export const applyCustomerFilter = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    if (isAdminUser(req.user)) {
      (req as any).customerFilter = null; // null means no filter (all access)
      next();
      return;
    }
    ...
    if (customerIds.length === 0) {
      ...
      (req as any).customerFilter = { id: { in: [] } };  // Use 'id' for Customer model
      console.log(`User ${req.user.email} has no customer assignments`);
    } else {
      (req as any).customerFilter = { id: { in: customerIds } };  // Use 'id' for Customer model
 ...
 */
export const applyBookingFilter = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    if (customerIds.length === 0) {
      (req as any).bookingFilter = { id: { in: [] } };
      console.log(`User ${req.user.email} has no accessible bookings`);
    } else {
      (req as any).bookingFilter = { customerId: { in: customerIds } };
 ...
 */
export const applyInvoiceFilter = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
 ...
 */
export const canAccessInvoice = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
 ...
 */
export async function getUserAssignmentInfo(userId: string) {
  try {
    const assignments = await prisma.customer_assignments.findMany({
      where: {
        userId,
...
export async function assignCustomerToUser(
  customerId: string,
  userId: string,
  assignedRole: string = 'SALES',
  commissionRates?: Partial<{
 ...
 */
export async function unassignCustomerFromUser(
  customerId: string,
  userId: string,
  assignedRole: string = 'SALES'
) {

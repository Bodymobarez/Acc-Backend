...
export class CommissionVerificationService {
   ...
   */
  async verifyBookingUserRelationships(bookingId: string): Promise<{
    valid: boolean;
    booking: any;
    creator: any;
    bookingAgent: any;
    customerServiceAgent: any;
    relationships: {
      hasCreator: boolean;
      hasBookingAgent: boolean;
      hasCustomerService: boolean;
      creatorIsUser: boolean;
      agentIsEmployee: boolean;
      csIsEmployee: boolean;
    };
    issues: string[];
  }> {
    const issues: string[] = [];
   ...
   */
  async verifyCommissionCalculations(bookingId: string): Promise<{
    valid: boolean;
    booking: any;
    stored: {
      grossProfit: number;
    ...
    if (differences.agentCommission > epsilon) {
      issues.push(
        `Agent commission mismatch: Stored ${stored.agentCommissionAmount}, ` +
    ...
    if (differences.csCommission > epsilon) {
      issues.push(
        `CS commission mismatch: Stored ${stored.csCommissionAmount}, ` +
    ...
    if (differences.totalCommission > epsilon) {
      issues.push(
        `Total commission mismatch: Stored ${stored.totalCommission}, ` +
    ...
    if (differences.vatAmount > epsilon) {
      issues.push(
        `VAT amount mismatch: Stored ${stored.vatAmount}, ` +
    ...
    if (differences.netProfit > epsilon) {
      issues.push(
        `Net profit mismatch: Stored ${stored.netProfit}, ` +
   ...
   */
  async getEmployeeCommissionSummary(employeeId: string): Promise<{
    employee: any;
    user: any;
    asBookingAgent: {
      bookings: any[];
    ...
    const agentTotalCommission = asBookingAgent.reduce(
      (sum, b) => sum + (b.agentCommissionAmount || 0),
      0
    );

    const csTotalCommission = asCustomerService.reduce(
      (sum, b) => sum + (b.csCommissionAmount || 0),
      0
    );
   ...
   */
  async verifyCommissionRates(bookingId: string): Promise<{
    valid: boolean;
    booking: any;
    agentRateCheck: {
      employeeDefaultRate: number | null;
      bookingRate: number;
      matches: boolean;
    };
    csRateCheck: {
    ...
    if (!agentMatches && agentEmployee) {
      issues.push(
        `Agent commission rate mismatch: Employee default ${agentDefaultRate}%, ` +
    ...
    if (!csMatches && csEmployee) {
      issues.push(
        `CS commission rate mismatch: Employee default ${csDefaultRate}%, ` +
   ...
   */
  async findAllCommissionIssues(): Promise<{
    totalBookings: number;
    bookingsWithIssues: number;
    issues: Array<{
      bookingId: string;
    ...
    const bookingsWithIssues: Array<{
      bookingId: string;
      bookingNumber: string;
      problems: string[];
    }> = [];

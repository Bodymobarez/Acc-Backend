import { VATCalculation, CommissionCalculation } from '../types';
 ...
 */
export function calculateVATForUAE(
  saleAmount: number,
  costAmount: number,
  vatRate: number = 5.0,
  serviceType?: string
 ...
 */
export function calculateVATForNonUAE(
  saleAmount: number,
  costAmount: number,
  vatRate: number = 5.0
): VATCalculation {
 ...
 */
export function calculateVATOnProfit(
...
export function calculateCommissions(
  grossProfit: number,  // Changed from netProfit to grossProfit
  agentCommissionRate: number = 0,
  csCommissionRate: number = 0
 ...
 */
export function generateFileNumber(prefix: string = 'FILE', sequence: number): string {
  const paddedSequence = sequence.toString().padStart(6, '0');
  const year = new Date().getFullYear();
  return `${prefix}-${year}-${paddedSequence}`;
}

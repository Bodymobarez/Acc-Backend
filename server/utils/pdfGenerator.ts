...
import { invoices, files, company_settings } from '@prisma/client';
...
export async function generateInvoicePDF(
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (customer.addressLine2) doc.text(customer.addressLine2);
      doc.text(`${customer.city}, ${customer.country}`);
      doc.text(`Phone: ${customer.phone}`);
      doc.text(`Email: ${customer.email}`);
      if (customer.taxNumber) doc.text(`TRN: ${customer.taxNumber}`);
      ...
      doc.text(
        ...
        leftColumn + tableWidths[0] + tableWidths[1],
        contentTop,
        { width: tableWidths[2], align: 'right' }
      );

 ...
 */
export const trackActivity = async (req, activityData) => {
    try {
        if (!req.user)
            return;
 ...
 */
export const activityTrackerMiddleware = (entityType, getEntityId) => {
 ...
 */
export const trackLogin = async (userId, email, ipAddress, userAgent, success) => {
    try {
        await prisma.activity_logs.create({
            data: {
                id: nanoid(),

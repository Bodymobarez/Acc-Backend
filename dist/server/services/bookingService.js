...
export class BookingService {
    ...
    async createBooking(input) {
        ...
        if (input.isUAEBooking && input.vatApplicable) {
            ...
            if (input.serviceType === 'FLIGHT') {
                ...
                const commissionCalc = calculateCommissions(commissionBase, agentCommissionRate, csCommissionRate);
    ...
    async updateCommissions(bookingId, input) {
        ...
        if (booking.isUAEBooking && booking.vatApplicable) {
            ...
            commissionCalc = calculateCommissions(commissionBase, input.agentCommissionRate || 0, input.csCommissionRate || 0);
    ...
    async updateBooking(id, data) {
        try {
            ...
            if (data.costAmount || data.saleAmount || data.costCurrency || data.saleCurrency ||
                ...
                const commissionCalc = calculateCommissions(vatCalc.grossProfit, agentCommissionRate, csCommissionRate);
    ...
    async recalculateBookingCosts(bookingId) {
        ...
        const totalCostInAED = booking.booking_suppliers.reduce((sum, supplier) => sum + supplier.costInAED, booking.costInAED // Include main supplier cost
        );
        ...
        const commissionBase = booking.isUAEBooking && booking.vatApplicable
            ? vatCalc.grossProfit
            : grossProfit;
        const commissionCalc = calculateCommissions(commissionBase, booking.agentCommissionRate || 0, booking.csCommissionRate || 0);

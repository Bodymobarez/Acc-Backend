...
export class ReportService {
    ...
    async getFinancialSummary(filters) {
        ...
        const costEntries = journalEntries.filter(e => ['BOOKING_COST', 'SUPPLIER_PAYMENT'].includes(e.transactionType));
        const costs = costEntries.reduce((sum, e) => sum + e.lines.filter(l => l.type === 'DEBIT').reduce((s, l) => s + Number(l.amount), 0), 0);
        // Calculate operating expenses (commissions, salaries, overhead)
        const expenseEntries = journalEntries.filter(e => ['COMMISSION_AGENT', 'COMMISSION_CS', 'EXPENSE', 'SALARY'].includes(e.transactionType));
        const operatingExpenses = expenseEntries.reduce((sum, e) => sum + e.lines.filter(l => l.type === 'DEBIT').reduce((s, l) => s + Number(l.amount), 0), 0);
     ...
     */
    async getRevenueTimeSeries(filters) {
     ...
     */
    async getProfitLossStatement(filters) {
    ...
    async getCashFlowStatement(filters) {
        ...
        const cashPayments = payments.reduce((sum, p) => sum + p.lines.filter(l => l.type === 'DEBIT').reduce((s, l) => s + Number(l.amount), 0), 0);
        return {
            period: { startDate, endDate },
            operatingActivities: {
                cashReceipts,
    ...
    async getAgingReport(filters) {
        ...
        invoices.forEach(inv => {
            ...
            if (daysOverdue <= 30)
                ...
                aging.overdue30.push(item);
            else if (daysOverdue <= 90)
                aging.overdue60.push(item);
            else
                aging.overdue90.push(item);
        });
        return {
            summary: {
     ...
     */
    async getCommissionsBreakdown(filters) {
        const { startDate, endDate } = filters;
        ...
        return {
            total: commissions.reduce((sum, c) => sum + c.lines.filter(l => l.type === 'DEBIT').reduce((s, l) => s + Number(l.amount), 0), 0),
    ...
    async getKPIs(filters) {
        ...
        const createKPI = (name, current, previous, target) => {
    ...
    async getBookingPerformance(filters) {
        ...
        return {
            ...
            byStatus,
            byServiceType,
            topServices: Object.entries(byServiceType).sort(([, a], [, b]) => b - a).slice(0, 5)
        };
    }
    /**
     * Get VAT report
    ...
    async getOperatingCashFlow(filters) {
        const metrics = await this.getFinancialSummary(filters);

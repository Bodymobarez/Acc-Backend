...
class NotificationService {
     ...
     */
    async notifyBookingAction(action, bookingId, bookingNumber, performedBy) {
        try {
            ...
            await this.createBulkNotifications(admins.map((a) => a.id), {
     ...
     */
    async notifyInvoiceAction(action, invoiceId, invoiceNumber, performedBy) {
        try {
            const admins = await prisma.users.findMany({
                where: {
                    role: { in: ['ADMIN', 'SUPER_ADMIN', 'ACCOUNTING'] },

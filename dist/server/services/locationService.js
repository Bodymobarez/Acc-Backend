...
class LocationService {
    ...
    async getCitiesByCountry(countryId) {
        try {
            ...
            const data = loadWorldData();
            // Find the country
            const country = data.find((c) => c.iso2 === countryId);
            if (!country) {
     ...
     * Create or get city
     */
    async upsertCity(name, countryId) {
     ...
     */
    async upsertHotel(name, cityId, address, phone, email, rating) {

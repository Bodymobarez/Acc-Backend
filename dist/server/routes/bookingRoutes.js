...
// Create booking
router.post('/', requirePermission('createBooking'), validate([
...
router.get('/', requirePermission('viewBookings'), applyBookingFilter, bookingController.getAll.bind(bookingController));
// Get booking by ID (with access check)
router.get('/:id', requirePermission('viewBookings'), canAccessBooking, bookingController.getById.bind(bookingController));
// Update booking (with access check)
router.put('/:id', requirePermission('editBooking'), canAccessBooking, bookingController.update.bind(bookingController));
// Update commissions (for accountants, with access check)
router.put('/:id/commissions', requirePermission('reviewBooking'), canAccessBooking, validate([
...
router.post('/:id/approve', requirePermission('reviewBooking'), canAccessBooking, bookingController.approve.bind(bookingController));
// Delete booking (with access check)
router.delete('/:id', requirePermission('deleteBooking'), canAccessBooking, bookingController.delete.bind(bookingController));
// Cancel booking with refund (creates refund booking and credit note)
router.post('/:id/cancel', requirePermission('editBooking'), canAccessBooking, bookingController.cancelWithRefund.bind(bookingController));
// Multi-Supplier Routes
// Add supplier to booking
router.post('/:id/suppliers', requirePermission('editBooking'), canAccessBooking, validate([
...
router.get('/:id/suppliers', requirePermission('viewBookings'), canAccessBooking, bookingController.getSuppliers.bind(bookingController));
// Remove supplier from booking
router.delete('/:id/suppliers/:supplierId', requirePermission('editBooking'), canAccessBooking, bookingController.removeSupplier.bind(bookingController));

...
export class InvoiceController {
    ...
    async updateStatus(req, res) {
        try {
            ...
            const invoice = await invoiceService.updateInvoiceStatus(id, status, paidDate ? new Date(paidDate) : undefined);
            res.json({
                success: true,
                data: invoice
            });
    ...
    async update(req, res) {
        try {
            ...
            if (Object.keys(updateData).length === 1 && 'status' in updateData) {
                const invoice = await invoiceService.updateInvoiceStatus(id, updateData.status, updateData.status === 'PAID' ? new Date() : undefined);
                res.json({
                    success: true,
                    data: invoice
                });

 ...
 */
export const searchHotelsByCity = async (req, res) => {
    try {
        ...
        const hotels = searchData.data.slice(0, 50).map((location) => ({
 ...
 */
export const searchHotelDetailsTripAdvisor = async (req, res) => {
    try {
 ...
 */
export const searchHotelDetails = async (req, res) => {
    try {
        const { hotelName, city } = req.body;
        if (!hotelName || !city) {
            return res.status(400).json({
 ...
 */
export const searchHotelsWithQuery = async (req, res) => {
    try {
        ...
        const allHotels = new Map(); // Use Map to avoid duplicates
        ...
        const hotels = Array.from(allHotels.values()).slice(0, 30).map((location) => {
